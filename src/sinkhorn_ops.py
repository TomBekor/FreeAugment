# from https://github.com/HeddaCohenIndelman/Learning-Gumbel-Sinkhorn-Permutations-w-Pytorch
...
def my_gumbel_sinkhorn(
    ...
    """
    # in case of a 2D matrix
    if len(log_alpha.size()) < 3:
    ...
    if noise_factor == 0:
        ...
        noise = to_var(
            my_sample_gumbel([n_samples * batch_size, n_rows, n_cols]) * noise_factor,
            device,
        )
    ...
    #
    if n_cols > n_rows:
        dummy_shape = list(my_log_alpha_w_noise.shape)
        dummy_shape[1] = n_cols - n_rows
        s = torch.cat(
            (my_log_alpha_w_noise, to_var(torch.full(dummy_shape, 0.0), device)), dim=1
        )
        new_n_rows = n_cols
        for b in range(batch_size):
            s[b, n_rows:new_n_rows, :n_cols] = log_epsilon

        my_log_alpha_w_noise = s  # extended with dummy rows

    assert my_log_alpha_w_noise.shape[-1] == my_log_alpha_w_noise.shape[-2]
    #
    #
    ...
    if ret_time:
        ...
        sink_time = starter.elapsed_time(ender)

    #
    # discard dummy rows from in case of unbalanced matching problem
    #
    if n_cols > n_rows:
        sink = sink[:, :n_rows, :]
    #
    ...
    if n_samples > 1 or squeeze is False:
        sink = sink.view(n_samples, batch_size, n_rows, n_cols)
        sink = torch.transpose(sink, 1, 0)
        log_alpha_w_noise = log_alpha_w_noise.view(
            n_samples, batch_size, n_rows, n_cols
        )
        log_alpha_w_noise = torch.transpose(log_alpha_w_noise, 1, 0)
    if ret_time:
        return sink, log_alpha_w_noise, sink_time
    return sink, log_alpha_w_noise


def my_new_gumbel_sinkhorn(
    log_alpha,
    temp=1.0,
    n_samples=1,
    noise_factor=1.0,
    ...
    #
    if n_cols > n_rows:
        dummy_shape = list(log_alpha_w_noise.shape)
        dummy_shape[1] = n_cols - n_rows
        s = torch.cat(
            (log_alpha_w_noise, to_var(torch.full(dummy_shape, 0.0), device)), dim=1
        )
        new_n_rows = n_cols
        for b in range(batch_size):
            s[b, n_rows:new_n_rows, :n_cols] = log_epsilon

        log_alpha_w_noise = s  # extended with dummy rows

    assert log_alpha_w_noise.shape[-1] == log_alpha_w_noise.shape[-2]
    #
    #
        ...
        noise = to_var(
            my_sample_gumbel(
                [
                    ...
                    log_alpha_w_noise.shape[-1],
                    log_alpha_w_noise.shape[-2],
                ]
            )
            * noise_factor,
            device,
    ...
    if n_cols > n_rows:
        sink = sink[:, :n_rows, :]
        log_alpha_w_noise = log_alpha_w_noise[:, :n_rows, :]
    #
    #
